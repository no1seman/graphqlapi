...
Module `models.lua` is used to load, remove and manipulate custom user GraphQL models. GraphQL models for this module must be written using GraphQL API module API and placed to project's subdirectory.
...
`init(dir_name)` - function to init `models` module including loading and apply custom GraphQL models provided by application,
...
`stop()` - function to deinit `models` module, it removes all models and cleanup all internal module variables including unload all modules that was required by all models functions. This behavior is needed to make possible hot-reload of all models.

### update_space_models()

`update_space_models(space_name)` - function to reload models related to provided `space_name`,
...
`apply_model(model)` - function to apply model,
...
`load_model(filename)` - function to load GraphQL API model from file,
...
`remove_model(filename)` - function is used to remove GraphQL API model from models internal cache,
...
`remove_model_by_space_name(space_name)` - function is used to remove all GraphQL API model(s) from models internal cache by spaces names,
...
`remove_all()` - function to remove all GraphQL API models from models internal cache.

### list_models()

`list_models()` - function is used to get array of path's of loaded models,
...
`list_loaded()` - function is used to get array of loaded models,
