...
g.test_init_stop = function()
    ...
    assert_enum_include('SpaceTruncateNames', {'good_space'})
    assert_enum_include('SpaceUpdateNames', {'good_space'})
    helpers.stop({ schema = 'spaces' })
    helpers.stop()
    good_space:drop()

    helpers.init()
    helpers.stop({ schema = box.NULL })
end
...
g.test_space_info = function()
    ...
    good_space:drop()
    bad_space:drop()
    helpers.space_info_remove({ schema = box.NULL, })

    good_space = create_space('good_space')
    helpers.space_info_init({include = {'good_space'}, exclude = {'bad_space'}})
    helpers.space_info_init({ schema = 'spaces', })
    bad_space = create_space('bad_space')
    assert_enum_include('SpaceInfoNames', {'good_space'})
    assert_enum_absent('SpaceInfoNames', {'bad_space'})
    good_space:drop()
    bad_space:drop()
    helpers.space_info_remove({ schema = 'spaces', })
    helpers.space_info_remove()
    assert_types_absent(types_list)
    assert_queries_absent(queries_list)
end
...
g.test_space_drop = function()
    ...
    good_space:drop()
    bad_space:drop()
    helpers.space_drop_remove({ schema = box.NULL, })

    good_space = create_space('good_space')
    bad_space = create_space('bad_space')
    helpers.space_drop_init({include = {'good_space'}, exclude = {'bad_space'}})
    helpers.space_drop_init({ schema = 'spaces', })
    assert_enum_include('SpaceDropNames', {'good_space'})
    assert_enum_absent('SpaceDropNames', {'bad_space'})
    good_space:drop()
    bad_space:drop()
    helpers.space_drop_remove({ schema = 'spaces', })
    helpers.space_drop_remove()
    assert_types_absent(types_list)
    assert_mutations_absent(mutations_list)
end
...
g.test_space_truncate = function()
    ...
    good_space:drop()
    bad_space:drop()
    helpers.space_truncate_remove({ schema = box.NULL, })

    good_space = create_space('good_space')
    bad_space = create_space('bad_space')
    helpers.space_truncate_init({include = {'good_space'}, exclude = {'bad_space'}})
    helpers.space_truncate_init({ schema = 'spaces', })
    assert_enum_include('SpaceTruncateNames', {'good_space'})
    assert_enum_absent('SpaceTruncateNames', {'bad_space'})
    good_space:drop()
    bad_space:drop()
    helpers.space_truncate_remove({ schema = 'spaces', })
    helpers.space_truncate_remove()
    assert_types_absent(types_list)
    assert_mutations_absent(mutations_list)
end
...
g.test_space_update = function()
    ...
    good_space:drop()
    bad_space:drop()
    helpers.space_update_remove({ schema = box.NULL, })

    good_space = create_space('good_space')
    bad_space = create_space('bad_space')
    helpers.space_update_init({include = {'good_space'}, exclude = {'bad_space'}})
    helpers.space_update_init({ schema = 'spaces', })
    assert_enum_include('SpaceUpdateNames', {'good_space'})
    assert_enum_absent('SpaceUpdateNames', {'bad_space'})
    good_space:drop()
    bad_space:drop()
    helpers.space_update_remove({ schema = 'spaces', })
    helpers.space_update_remove()
    assert_types_absent(types_list)
    assert_mutations_absent(mutations_list)
end
...
g.test_space_create = function()
    ...
    helpers.space_create_init({ schema = 'spaces', })
    t.assert_items_include(types.list_types(), types_list)
    t.assert_items_equals(operations.list_mutations(), mutations_list)
    helpers.space_create_remove({ schema = 'spaces', })
    helpers.space_create_remove()
    assert_types_absent(types_list)
    assert_mutations_absent(mutations_list)

    helpers.space_create_init()
    helpers.space_create_remove({ schema = box.NULL, })
end
