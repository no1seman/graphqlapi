...
g.test_load_model = function()
    ...
    ok, err = models.load_model(helper.shared.root..'/test/models/suite1/syntax_error.lua')
    t.assert_equals(ok, nil)
    t.assert_str_contains(err,
        'unexpected symbol near \'111\'')

    -- check file with missing model
    ok, err = models.load_model(helper.shared.root..'/test/models/suite1/missing_model.lua')
    t.assert_equals(ok, nil)
    t.assert_str_contains(err.err,
        'model must contain \'model\' function')

    -- check file with invalid spaces
    ok, err = models.load_model(helper.shared.root..'/test/models/suite1/invalid_spaces.lua')
    t.assert_equals(ok, nil)
    t.assert_str_contains(err.err,
        'model.spaces must be a table')

    -- check file with invalid space in spaces array
    ok, err = models.load_model(helper.shared.root..'/test/models/suite1/invalid_space.lua')
    t.assert_equals(ok, nil)
    t.assert_str_contains(err.err,
        'model.spaces item \'1\' must be a string')

    -- check file with missing spaces
    local model = models.load_model(helper.shared.root..'/test/models/suite1/missing_spaces.lua')
    t.assert_equals(type(model.model), 'function')

    -- check file with valid model
    model = models.load_model(helper.shared.root..'/test/models/suite1/valid_model.lua')
